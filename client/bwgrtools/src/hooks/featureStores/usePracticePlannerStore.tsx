...
interface PracticePlannerState {
	...
	addPractice: (e: FormEvent<HTMLFormElement>, practices: BwgrPractice[], cells: BwgrPracticeCell[], setNotification: React.Dispatch<React.SetStateAction<JSX.Element | null>>) => void;
...
export const usePracticePlannerStore = create<PracticePlannerState>()(
	devtools(
		(set) => ({
			...
			addPractice: (e: FormEvent<HTMLFormElement>, practices: BwgrPractice[], cells: BwgrPracticeCell[], setNotification: React.Dispatch<React.SetStateAction<JSX.Element | null>>) => {
				e.preventDefault();
				...
				const practiceId = pId as unknown as BwgrPracticeId;
				const testType = tType as Exclude<keyof BwgrPractice, "id" | "ability" | "skillType" | "cycle">;
				...
				if (name === "") {
					setNotification(<Notification text="Please enter aBwgrSkill name." severity="error" onClose={() => setNotification(null)} />);
				}
					...
					const practice: BwgrPracticePlaced = { practiceId, name, testType, hours };
					set(produce<PracticePlannerState>((state) => {
						const newCells = state.cells;
						state.cells = newCells.map((v, i) => {
							if (i === cellIndex) {
