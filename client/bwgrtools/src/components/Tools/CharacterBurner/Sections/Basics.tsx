...
export function Basics({ openModal }: { openModal: (name: CharacterBurnerModals) => void; }): JSX.Element {
	...
	const hasSpecialLifepath
		= lifepaths.some(lifepath => Array.isArray(lifepath.years) || (lifepath.companion && lifepath.companion.givesSkills));

	const hasSpecialSkills
		= skills.filter(charSkill => charSkill.name === "Any Skill" || charSkill.name === "Any Wise" || ruleset.getSkill(charSkill.id).subskillIds !== undefined).length > 0;
	...
	return (
		<GenericGrid columns={12} center extraBottomMargin>
			...
			<Grid item xs={12} sm={2}>
				<TextField label="Stride" value={ruleset.getStock(stock[0]).stride} fullWidth variant="standard" disabled />
			</Grid>
