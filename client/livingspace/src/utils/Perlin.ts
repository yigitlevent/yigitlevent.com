 ...
 * Converted to TypeScript by GitHub Copilot.
 *
 * Version 2012-03-09
 *
...
const Perm: number[] = new Array<number>(512);
const GradP: Grad[] = new Array<Grad>(512);
...
export function Simplex2(xin: number, yin: number): number {
	...
	const s: number = (xin + yin) * F2; // Hairy factor for 2D
	...
	const x2: number = x0 - 1 + 2 * G2; // Offsets for last corner in (x,y) unskewed coords
	const y2: number = y0 - 1 + 2 * G2;
	// Work out the hashed gradient indices of the three simplex corners
	i &= 255;
	j &= 255;
	const gi0: Grad = GradP[i + Perm[j]];
	const gi1: Grad = GradP[i + i1 + Perm[j + j1]];
	const gi2: Grad = GradP[i + 1 + Perm[j + 1]];
...
export function Perlin2(x: number, y: number): number {
	...
	let aX: number = Math.floor(x);
	let aY: number = Math.floor(y);
	...
	const n00: number = GradP[aX + Perm[aY]].dot2(x, y);
	const n01: number = GradP[aX + Perm[aY + 1]].dot2(x, y - 1);
	const n10: number = GradP[aX + 1 + Perm[aY]].dot2(x - 1, y);
	const n11: number = GradP[aX + 1 + Perm[aY + 1]].dot2(x - 1, y - 1);
...
export function Perlin3(x: number, y: number, z: number): number {
	...
	let aX: number = Math.floor(x);
	let aY: number = Math.floor(y);
	let aZ: number = Math.floor(z);

	// Get relative xyz coordinates of point within that cell
	x = x - aX; y = y - aY; z = z - aZ;
	// Wrap the integer cells at 255 (smaller integer period can be introduced here)
	aX = aX & 255; aY = aY & 255; aZ = aZ & 255;

	// Calculate noise contributions from each of the eight corners
	const n000: number = GradP[aX + Perm[aY + Perm[aZ]]].dot3(x, y, z);
	const n001: number = GradP[aX + Perm[aY + Perm[aZ + 1]]].dot3(x, y, z - 1);
	const n010: number = GradP[aX + Perm[aY + 1 + Perm[aZ]]].dot3(x, y - 1, z);
	const n011: number = GradP[aX + Perm[aY + 1 + Perm[aZ + 1]]].dot3(x, y - 1, z - 1);
	const n100: number = GradP[aX + 1 + Perm[aY + Perm[aZ]]].dot3(x - 1, y, z);
	const n101: number = GradP[aX + 1 + Perm[aY + Perm[aZ + 1]]].dot3(x - 1, y, z - 1);
	const n110: number = GradP[aX + 1 + Perm[aY + 1 + Perm[aZ]]].dot3(x - 1, y - 1, z);
	const n111: number = GradP[aX + 1 + Perm[aY + 1 + Perm[aZ + 1]]].dot3(x - 1, y - 1, z - 1);
