...
import type { BwgrLifepathId } from "./lifepath";
import type { BwgrQuestionId } from "./question";
import type { BwgrResourceId, BwgrResourceTypeId } from "./resource";
import type { BwgrSkillId } from "./skill";
import type { BwgrStockId } from "./stock";
import type { BwgrTraitId } from "./trait";


export type BwgrCharacterBurnerModals = "lp" | "randLp" | "re" | "geSk" | "geTr" | "qu" | "so";
export type BwgrShades = "B" | "G" | "W";

export interface BwgrCharacterAttribute {
  id: BwgrAbilityId;
  name: string;
  hasShade: boolean;
...
export interface BwgrCharacterSkill {
  id: BwgrSkillId;
  name: string;
...
export interface BwgrCharacterTrait {
  id: BwgrTraitId;
  name: string;
...
export interface BwgrCharacterResource {
  id: BwgrResourceId;
  name: string;
...
export interface BwgrCharacterStockLimits {
  beliefs: number;
  instincts: number;
  stats: {
...
export interface BwgrCharacterQuestion {
  id: BwgrQuestionId;
  name: string;
  question: string;
  answer: boolean;
...
export type BwgrCharacterTraitEffect =
  | { roundUp: BwgrAbilityId; }
  | { roundUp: "Mortal Wound"; }
  | { callOn: BwgrAbilityId; }
  | { callOn: BwgrSkillId; };

export type BwgrHuntingGroundsList = "Waste" | "Marginal" | "Typical" | "Plentiful" | "Untouched";

export interface BwgrPoints {
  total: number;
  spent: number;
  remaining: number;
}

export interface BwgrAbilityPoints {
  shade: BwgrShades;
  exponent: number;
}

export interface BwgrBurningCharacter {
  basics: {
    name: string;
    concept: string;
    gender: "Male" | "Female";
